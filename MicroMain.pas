...
begin
  case Act of
    ...
    2:
      SpawnCreature(2, FCamX + 1.5, FCamY + 1.5, nil, nil, 0);
    3:
...
begin
  ...
  for C in Creatures do
    if C.Alive and (C.Kind = 2) then
      if (Best = nil) or (C.Cult > Best.Cult) then
        Best := C;
  FSelected := Best;
end;
...
begin
  ...
  if FSelected <> nil then
    Start := Creatures.IndexOf(FSelected) + 1;
  for I := 0 to Creatures.Count - 1 do
    if Creatures[(Start + I) mod Creatures.Count].Alive and
      (Creatures[(Start + I) mod Creatures.Count].Kind = 2) then
    begin
...
begin
  ...
  try
    ...
    SampleT := 0;
    for I := 1 to 1280 do
      AddPlant(Random(GW), Random(GH), 0.3 + Random * 0.6);
    for G := 1 to 8 do
    begin
      X := Random(GW);
      Y := Random(GH);
...
begin
  ...
  if X < FPanelW then
  begin
    if (X > PANELW - 8) and (FPanelH > ClientHeight) then
    begin
      if (Y + FPanelScroll) * ClientHeight div FPanelH < FPanelScroll +
    ...
    for I := 0 to High(FBtns) do
      if PtInRect(FBtns[I].R, Point(X, Y)) then
      begin
        if (FBtns[I].Id >= BID_CFGDEC) and (FBtns[I].Id < BID_CFGDEC + CN) then
        begin
          ConfigAdjust(FBtns[I].Id - BID_CFGDEC, -1);
          Invalidate;
          Exit;
        end;
        if (FBtns[I].Id >= BID_CFGINC) and (FBtns[I].Id < BID_CFGINC + CN) then
        begin
        ...
        case FBtns[I].Id of
          ...
          BID_RELIEF:
            begin
              SetRelief(not ReliefOn);
              Invalidate
            end;
          BID_CHRON:
            begin
              FChronShow := not FChronShow;
              Invalidate
            end;
          BID_ERE:
            if PassEre then
            begin
              ChronAdd(CK_TECH,
                Format(L(117), [EreCourante, ERE_NOM(EreCourante)]));
    ...
    case FTool of
      ...
      TOOL_SEED:
        AddPlant(WP.X + (Random - 0.5) * 2, WP.Y + (Random - 0.5) * 2,
          0.25 + Random * 0.35);
      TOOL_HERB:
        SpawnCreature(0, WP.X, WP.Y, nil, nil, 0);
      TOOL_PRED:
        SpawnCreature(1, WP.X, WP.Y, nil, nil, 0);
      TOOL_SAP:
        SpawnCreature(2, WP.X, WP.Y, nil, nil, 0);
    end;
  finally
    FSimCS.Leave;
  end;
...
begin
  ...
  if (FVue = 0) and (P.X < FPanelW) then
  begin
    FPanelScroll := ClampInt(FPanelScroll - WheelDelta, 0,
      FPanelH - ClientHeight);
    if FPanelScroll < 0 then
      FPanelScroll := 0;
    Invalidate;
    Result := True;
  end
  else if (FVue <> 2) and (P.X >= FVP.Left) and FStarted then
  begin
...
begin
  ...
  begin
    SoundOn := not SoundOn;
    if SoundOn then
      AudioSetMasterVolume(0.85)
    else
      AudioSetMasterVolume(0);
    Key := 0;
  end;
  ...
  begin
    ...
    if Key = Ord('C') then
    begin // ★Phase C : triche — élire un chef
      Key := 0;
      var
        V2: TCity;
      var
      ...
      begin
        V2 := Cities[0];
        BD := 1E9;
        for var K2 := 0 to Cities.Count - 1 do
        begin
        ...
        if EloireChef(V2) then
          Invalidate; // la ville la plus proche de la caméra
      end;
    end;

    if Key = Ord('R') then
    begin // ★ triche : route forcée (ou achevée)
      Key := 0;
      var
        R: TRoad;
      ...
      begin
        ...
        begin
          ...
          R.A := Cities[0];
          R.B := Cities[1];
          R.Chemin := CheminRoute(Trunc(R.A.X), Trunc(R.A.Y), Trunc(R.B.X),
            Trunc(R.B.Y));
    ...
    begin
      ...
      if CanPassEre then
        MsgP := L(120)
      else if EreCourante >= MaxEraContent then
        MsgP := L(121)
      else
        MsgP := L(122);
      Toast(Format('%s | %s | villes %d · routes %d',
        [EreLabel, Format(L(32), [CountTechEre(EreCourante),
        IfThen(EreCourante = 1, 7, 8), CountS, CountInno, InnoTotal]),
    ...
    if Key = Ord('U') then
    begin // ★ triche : fonder une ville ici
      Key := 0;
      var
        H: THut;
      var
...
begin
  ...
  if FVue = 2 then
    ...
    FPanelW := PANELW;
  if (FVP.Width > 0) and (FVP.Height > 0) then
  begin
  ...
  try
    ...
    try
      ...
      begin
        ...
        if FPanelBM.Height < FPanelH then
          FPanelBM.SetSize(FPanelW, FPanelH);
        if FPanelBM.Width <> FPanelW then
          FPanelBM.SetSize(FPanelW, FPanelH);
        DrawPanel(FPanelBM.Canvas, FPanelBM.Height);

        if FVue = 0 then
        begin
          FPanelScroll := ClampInt(FPanelScroll, 0,
            Max(0, FPanelH - ClientHeight));
          Canvas.CopyRect(Rect(0, 0, PANELW, ClientHeight), FPanelBM.Canvas,
            Rect(0, FPanelScroll, PANELW, FPanelScroll + ClientHeight));
        end;

        if (FVue = 0) and (FPanelH > ClientHeight) then
        begin
          ...
          if CurH < 30 then
            CurH := 30;
          SJ := (ClientHeight - CurH) * FPanelScroll div
            (FPanelH - ClientHeight);
    ...
    if (not FStarted) and (FVue = 0) then
    begin
      Canvas.Brush.Style := bsSolid;
      Canvas.Brush.Color := Col(14, 17, 12);
      Canvas.FillRect(FVP);

      DrawLogo(Canvas, FVP.Left + (FVP.Width - 300) div 2,
      ...
      Canvas.Font.Color := Col(230, 224, 205);
      Canvas.Brush.Style := bsClear;
      Canvas.TextOut(FVP.Left + (FVP.Width - Canvas.TextWidth('Microcosme'))
        div 2, FVP.Top + FVP.Height div 2 + 140, 'Microcosme');

      BR := Rect(FVP.Left + FVP.Width div 2 - 100, FVP.Top + FVP.Height div 2 +
        210, FVP.Left + FVP.Width div 2 + 100,
        FVP.Top + FVP.Height div 2 + 252);
      AddBtn(BR, L(0), BID_START, True);
      Canvas.Brush.Style := bsSolid;
      Canvas.Brush.Color := Col(208, 167, 92);
      Canvas.FillRect(BR);
      Canvas.Brush.Style := bsClear;
      Canvas.Font.Name := 'Segoe UI';
      Canvas.Font.Size := 10;
...
begin
  ...
  if FileExists(SDiag) then
    DeleteFile(PChar(SDiag));
  ...
  for I := 0 to Length(Buckets) - 1 do
    Buckets[I] := TList<TCreature>.Create;
