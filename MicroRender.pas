...
{ Microcosme — rendu terrain interpolé (îlots), poissons, pirogues,
  huttes (feu, réserves, champs), carte mentale, liens mentors,
  bulles vocales, cerveau 24→9→9, carnet DÉFILABLE + lexique + culture
  + technologies. }
...
uses
  ...
  MicroTypes, MicroBrain,MicroConfig,MicroEvo,MicroIno,MicroChrono,MicroEre;
...
uses MicroSim, MicroRenderPro,MicroHelp;
...
const
  JOURS_ANNEE = 24;   // 4 saisons de 6 jours — à votre goût : 16 (rapide) .. 48 (longues ères)
...
begin
  ...
  for PY := 0 to H - 1 do begin
    ...
    for PX := 0 to W - 1 do begin
      ...
      PrevE := Ev;
           // — relief : ombrage selon la pente (lumière du nord-ouest) —
...
var S, OX, OY, VX0, VY0, VX1, VY1: Double;
   W, H, PX, PY, RR, K, I, MO, MK, Flags: Integer;
...
begin
  ...
  with FWorld.Canvas do begin
    ...
    for I := 0 to Huts.Count - 1 do begin
      ...
      // ★ERE5 — corps : maison de pierre (ère 2+), hutte d'origine en ère 1
      if FEra >= 2 then begin
        ...
        Pen.Width := 1;
        if FEra >= 3 then Brush.Color := Col(150, 148, 140)     // ère 3 : pierre taillée claire
        ...
        Polygon(Poly);
        if HHut.Fire then begin                                 // fanion doré sur la hutte au feu
    ...
    if Warm > 0 then
      AlphaFill(FWorld.Canvas, W, H, FTintW, Col(255, 150, 70), Trunc(Warm * 15));
          // <<< SAISONS : hiver bleuté · été doré (par-dessus nuit et couchant)
    Froid := SaisonFroid;
    if Froid > 0.03 then
...
begin
  ...
  XIn := 158; XH1 := Round(W * 0.40); XH2 := Round(W * 0.62); XOut := W - 158;

  // — liaisons entrée → cachée 1 —
  for I := 0 to NIN - 1 do
    for J := 0 to NHID - 1 do begin
      Wv := FSelected.Net[I * NHID + J];
  ...
  // — voie directe entrée → sortie (pointillés) —
  for I := 0 to NIN - 1 do
    for O := 0 to NOUT - 1 do begin
      Wv := FSelected.Net[IDX_SO + I * NOUT + O];
      if Abs(Wv) < 0.5 then Continue;
...
begin
  ...
  C.FillRect(Rect(0, 0, PANELW, H));
  C.Pen.Style := psSolid; C.Pen.Color := Col(38, 43, 33); C.Pen.Width := 1;
  C.MoveTo(PANELW - 1, 0);
  C.LineTo(PANELW - 1, H);

  // — en-tête : date + badge d'ère + critères (l'ancien bloc dupliqué est fusionné) —
  Y := 18;
  ...
  // ★ERE5 — badge d'ère (Georgia doré, écho du bouton de passage)
  C.Font.Name := 'Georgia'; C.Font.Size := 11; C.Font.Style := [fsItalic, fsBold];
  ...
  C.FillRect(Rect(20, Y + 8, PANELW - 20, Y + 10));
  C.Brush.Color := Col(208, 167, 92);
  C.FillRect(Rect(20, Y + 8, 20 + Trunc((PANELW - 40) * FDayT), Y + 10));
  Inc(Y, 26);
  ...
  for I := 0 to 4 do begin
    ...
    S := IntToStr(V);
    C.Font.Color := Col(230, 224, 205);
    C.TextOut(PANELW - 20 - C.TextWidth(S), Y, S);
    Inc(Y, 17);
  end;
  Inc(Y, 6);
  ...
  C.Font.Size := 9; C.Brush.Style := bsClear;
  C.Font.Color := Col(208, 167, 92);
  if GN > 0 then
  ...
  // ★ERE5 — Technologies : toutes les ères ouvertes, sous-titres d'ère, TechNom
  Section('Technologies');
  C.Font.Size := 9;
  LastEra := 0;
  for I := 0 to TECH_COUNT - 1 do begin
    TT := TECHBASE[I].Code;
    if TECHBASE[I].Era > EreCourante then Continue;      // ère à venir : encore secrète
    ...
    C.TextOut(20, Y, TechNom(TT));                       // ★ERE5 (TECHNAMES = ère 1 seulement)
    C.Font.Color := Col(208, 167, 92);
    C.TextOut(96, Y, Gl);
    C.Font.Color := Col(139, 138, 116);
    S := Format('%d%%', [Car]);
    ...
    C.Brush.Style := bsSolid;
    C.Brush.Color := Col(32, 37, 28);
    C.FillRect(Rect(20, Y, PANELW - 20, Y + 3));
    C.Brush.Color := Col(240, 180, 95);
    C.FillRect(Rect(20, Y, 20 + Trunc((PANELW - 40) * Car / 100), Y + 3));
    C.Brush.Style := bsClear;
    Inc(Y, 14);
  end;
  Inc(Y, 6);
  ...
  for I := 0 to 3 do begin
    ...
    C.Brush.Style := bsSolid;
    C.Brush.Color := Col(32, 37, 28);
    C.FillRect(Rect(44, Y, PANELW - 46, Y + 3));
    if Has then begin
      C.Brush.Color := Col(201, 106, 69);
      C.FillRect(Rect(44, Y, 44 + Trunc((PANELW - 90) * PP / 100), Y + 3));
    end;
    Inc(Y, 5);
    C.Brush.Style := bsSolid;
    C.Brush.Color := Col(32, 37, 28);
    ...
    if Has then begin
      ...
      C.FillRect(Rect(44, Y, 44 + Trunc((PANELW - 90) * FF / 100), Y + 3));
    end;
    C.Brush.Style := bsClear;
    C.Font.Color := Col(100, 105, 88);
    C.TextOut(PANELW - 46, Y - 2, 'pr/n');
    Inc(Y, 9);
  end;
  Inc(Y, 4);
  ...
  if Length(InnoLog) = 0 then begin
    ...
    for I := Max(0, High(InnoLog) - 11) to High(InnoLog) do begin
      ...
      C.Font.Color := Col(139, 138, 116);
      C.TextOut(110, Y, 'par ' + InnoLog[I].Who);
      C.Font.Color := Col(100, 105, 88);
    ...
    if not Mesuring then begin
      AddBtn(Rect(PANELW - 84, Y - FPanelScroll + 2, PANELW - 20, Y - FPanelScroll + 24),
             'fiche', BID_INFO, False);
      C.Brush.Style := bsSolid; C.Pen.Style := psSolid; C.Pen.Width := 1;
      C.Pen.Color := Col(38, 43, 33);
      C.Brush.Color := Col(20, 24, 17);
      C.Rectangle(PANELW - 84, Y + 2, PANELW - 20, Y + 24);
      C.Font.Name := 'Segoe UI'; C.Font.Size := 9; C.Font.Style := [];
      C.Brush.Style := bsClear;
      C.Font.Color := Col(139, 138, 116);
      C.TextOut(PANELW - 84 + ((64 - C.TextWidth('fiche')) div 2), Y + 8, 'fiche');
    end;
    Inc(Y, 20);
    C.Font.Name := 'Segoe UI'; C.Font.Size := 8; C.Font.Style := [];
    C.Font.Color := Col(139, 138, 116);
    ...
    C.TextOut(20, Y, S);
    Inc(Y, 20);
    DrawBar(C, 20, Y, PANELW - 74, FSelected.Energy / FSelected.MaxE, Col(228, 220, 190));
    C.Font.Color := Col(139, 138, 116); C.Brush.Style := bsClear;
    C.TextOut(PANELW - 50, Y - 4, IntToStr(Trunc(Max(0, FSelected.Energy))));
    Inc(Y, 12);
    C.TextOut(20, Y, 'vitesse');
    DrawBar(C, 94, Y + 2, PANELW - 148, (FSelected.Sp - 2.2) / 3.2, Col(208, 167, 92));
    Inc(Y, 12);
    C.TextOut(20, Y, 'perception');
    DrawBar(C, 94, Y + 2, PANELW - 148, (FSelected.Se - 4) / 8, Col(208, 167, 92));
    Inc(Y, 12);
    C.TextOut(20, Y, 'taille');
    DrawBar(C, 94, Y + 2, PANELW - 148, (FSelected.Sz - 0.6) / 0.9, Col(208, 167, 92));
    Inc(Y, 12);
    if FSelected.Kind = 2 then begin
      C.TextOut(20, Y, 'savoir');
      DrawBar(C, 94, Y + 2, PANELW - 148, FSelected.Cult, Col(157, 187, 107));
      Inc(Y, 12);
      S := '';
      ...
      DrawBrain(C, Y);
      if not Mesuring then
        AddBtn(Rect(20, Y - FPanelScroll, PANELW - 20, Y - FPanelScroll + BRH),
               '', BID_BRAIN, False);
      Inc(Y, BRH + 22);
    end;
  end;
    ...
    Section('Commandes');
  // ★ERE5 — le bouton doré : visible UNIQUEMENT quand CanPassEre est vrai
  if CanPassEre then begin
    BX := 20;
    Btn('entrer dans l''ère suivante', BID_ERE, PANELW - 40, True);
    Inc(Y, 32);
  end;
  // rangée 1 : le temps (4 boutons égaux)
  BX := 20;
  if FRunning then Btn('II', BID_PLAY, (PANELW - 58) div 4, True)
              else Btn('>', BID_PLAY, (PANELW - 58) div 4, False);
  Btn('1x', BID_S1, (PANELW - 58) div 4, FSpeed = 1);
  Btn('2x', BID_S2, (PANELW - 58) div 4, FSpeed = 2);
  Btn('4x', BID_S4, (PANELW - 58) div 4, FSpeed = 4);
  Inc(Y, 32);

  // rangée 2 : les outils (5 boutons égaux)
  BX := 20;
  Btn('vue', BID_TI, (PANELW - 64) div 5, FTool = TOOL_INSPECT);
  Btn('sem', BID_TS, (PANELW - 64) div 5, FTool = TOOL_SEED);
  Btn('her', BID_TH, (PANELW - 64) div 5, FTool = TOOL_HERB);
  Btn('pré', BID_TP, (PANELW - 64) div 5, FTool = TOOL_PRED);
  Btn('sap', BID_TSA, (PANELW - 64) div 5, FTool = TOOL_SAP);
  Inc(Y, 32);

  // rangée 3 : sauvegarde (2 boutons égaux)
  BX := 20;
  Btn('Sauver', BID_SAVE, (PANELW - 46) div 2, False);
  Btn('Charger', BID_LOAD, (PANELW - 46) div 2, False);
  Inc(Y, 32);

  // rangée 4 : pleine largeur
  BX := 20;
  Btn('Nouveau monde', BID_NEW, PANELW - 40, False);
  Inc(Y, 32);

  // rangée 5 : aide + réglages + annales (3 boutons égaux)
  BX := 20;
  Btn('aide (F1)', BID_HELP, (PANELW - 52) div 3, False);
  Btn('réglages', BID_CFGSHOW, (PANELW - 52) div 3, FCfgShow);
  Btn('annales', BID_CHRON, (PANELW - 52) div 3, FChronShow);
  Inc(Y, 32);

  // rangée 6 : affichages (2 boutons égaux)
  BX := 20;
  Btn('3D', BID_G3D, (PANELW - 46) div 2, False);
  Btn('relief', BID_RELIEF, (PANELW - 46) div 2, FRelief);
  Inc(Y, 32);
  if FCfgShow then begin
    ...
    BX := 20;
    Btn('tout par défaut', BID_CFGDEF, PANELW - 60, False);
    Inc(Y, 36);
  end;
  ...
  if FThumb.Width > 0 then begin
    C.StretchDraw(Rect(20, Y, 120, Y + 62), FThumb);
    C.Brush.Style := bsSolid;
    for I := 0 to Creatures.Count - 1 do begin
      ...
      C.FillRect(Rect(20 + Trunc(Creatures[I].X / GW * 100) - 1,
                      Y + Trunc(Creatures[I].Y / GH * 62) - 1,
                      20 + Trunc(Creatures[I].X / GW * 100) + 1,
                      Y + Trunc(Creatures[I].Y / GH * 62) + 1));
    end;
  end;
  Inc(Y, 70);
