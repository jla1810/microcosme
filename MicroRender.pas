...
const
  BaseCol: array [0 .. 6] of array [0 .. 2] of Integer = ((16, 38, 40),
    (37, 73, 67), (186, 168, 122), (110, 131, 79), (63, 89, 57),
    (118, 113, 100), (208, 206, 192));
begin
  W := GW * SS;
  H := GH * SS;
  ...
  begin
    ...
    begin
      ...
      E01 := Elev[Y1 * GW + X0];
      E11 := Elev[Y1 * GW + X1];
      Ev := (E00 + (E10 - E00) * FX) * (1 - FY) + (E01 + (E11 - E01) * FX) * FY;
      H00 := Humid[Y0 * GW + X0];
      H10 := Humid[Y0 * GW + X1];
      H01 := Humid[Y1 * GW + X0];
      H11 := Humid[Y1 * GW + X1];
      Hu := (H00 + (H10 - H00) * FX) * (1 - FY) + (H01 + (H11 - H01) * FX) * FY;
      if Ev < 0.34 then
        T := T_DEEP
      else if Ev < 0.40 then
        T := T_SHAL
      else if Ev < 0.435 then
        T := T_SAND
      else if Ev < 0.68 then
      begin
        if Hu > 0.57 then
          T := T_FOR
        else
      ...
      PrevE := Ev;
      if FRelief and (X0 > 0) and (X0 < GW - 1) and (Y0 > 0) and (Y0 < GH - 1)
      then
      begin
      ...
      I := Y0 * GW + X0;
      if (T = T_SHAL) and (X0 > 0) and (X0 < GW - 1) and (Y0 > 0) and
        (Y0 < GH - 1) and ((TerrType[I - 1] >= T_SAND) or
        (TerrType[I + 1] >= T_SAND) or (TerrType[I - GW] >= T_SAND) or
        (TerrType[I + GW] >= T_SAND)) then
        M := M * 1.45;
...
procedure AlphaFill(C: TCanvas; W, H: Integer; Tint: TBitmap;
  Clr: TColor; A: Byte);
var
  BF: TBlendFunction;
begin
...
procedure RenderWorld;
{ FIX5 : routes+villes déléguées à MicroVilles (fin du collage fantôme) ;
  Exode : les huttes de banlieue (Ville <> nil, > 4.5 cases du centre)
  sont dessinées comme de vraies maisons ;
  Faune : mouton (Kind 5, toison claire, gabarit herbivore) et
  ours (Kind 4, brun, gabarit prédateur) rendus via ProRender 0/1. }
var
  S, OX, OY, VX0, VY0, VX1, VY1: Double;
  ...
  begin
    Result := (X > VX0 - M) and (X < VX1 + M) and (Y > VY0 - M) and
      (Y < VY1 + M);
  end;
...
begin
  ...
  with FWorld.Canvas do
  begin
    Brush.Style := bsSolid;
    Brush.Color := Col(11, 14, 11);
    FillRect(Rect(0, 0, W, H));
    StretchDraw(Rect(Trunc(OX), Trunc(OY), Trunc(OX + GW * S),
      Trunc(OY + GH * S)), FTerrain);

    // ===== ROUTES — sous tout le bâti (FIX5 : enfin dessinées) =====
    DrawRoads(FWorld.Canvas, OX, OY, S, VX0, VY0, VX1, VY1);

    // ===== HUTTES ISOLÉES + BANLIEUES =====
    for I := 0 to Huts.Count - 1 do
    begin
      HHut := Huts[I];
      if not InV(HHut.X, HHut.Y, 10) then
        Continue;
      if HHut.Ville <> nil then
      begin
        // ★Exode — dans la couronne dense (< 4.5 cases) : la ville dessine
        // en stylisé ; au-delà (banlieue, 8-20 cases) : vraie maison ci-dessous
        if Sqr(HHut.X - HHut.Ville.X) + Sqr(HHut.Y - HHut.Ville.Y) < Sqr(4.5)
        then
          Continue;
      end;
      ...
      begin
        ...
        Brush.Style := bsClear;
        RR := Trunc(S * 2);
        Arc(PX - RR, PY - RR, PX + RR, PY + RR, PX, PY - RR,
      ...
      begin
        ...
        Pen.Width := 1;
        if FEra >= 3 then
          Brush.Color := Col(150, 148, 140)
        else
          Brush.Color := Col(123, 117, 108);
        if FEra >= 6 then
        begin
          // la Renaissance peint ses façades
          case (Trunc(HHut.X) * 7 + Trunc(HHut.Y) * 13) mod 4 of
            0:
              Brush.Color := Col(198, 168, 130); // ocre
            1:
              Brush.Color := Col(172, 178, 150); // sauge
            2:
              Brush.Color := Col(186, 152, 148); // rose de Sienne
          else
            Brush.Color := Col(158, 170, 182); // azur pâle
          end;
        end;
        ...
        begin // fanion doré
          ...
          Poly[1] := Point(PX + Max(3, RR), PY - RR * 2 - Max(2, RR) + Max(2,
            RR div 2));
          Poly[2] := Point(PX, PY - RR * 2 - Max(2, RR) + Max(3, RR));
          Polygon(Poly);
        end;
        // colonne (Antiquité), donjon (Moyen Âge)
        if (FEra >= 4) and HHut.Fire and PeopleHas(teCite) then
        begin
          Brush.Style := bsSolid;
          Pen.Style := psSolid;
      ...
      for MK := 0 to Marks.Count - 1 do
        if Sqr(Marks[MK].X - HHut.X) + Sqr(Marks[MK].Y - HHut.Y) < 9 then
        begin
    ...
    // ===== CAMP ANCESTRAL =====
    if FHomeSet and (Huts.Count = 0) then
    begin
      PX := Trunc(OX + FHomeX * S);
      PY := Trunc(OY + FHomeY * S);
      Brush.Style := bsClear;
      Pen.Style := psDot;
      Pen.Width := 1;
      Pen.Color := Col(120, 130, 100);
      RR := Trunc(S * 3);
      Ellipse(PX - RR, PY - RR, PX + RR, PY + RR);
      Pen.Style := psSolid;
    end;
    ...
    Brush.Style := bsSolid;
    Pen.Style := psClear;
    for I := 0 to Fishes.Count - 1 do
    begin
      FF := Fishes[I];
      if not InV(FF.X, FF.Y, 2) then
        Continue;
      PX := Trunc(OX + FF.X * S);
      PY := Trunc(OY + FF.Y * S);
      RR := Max(2, Trunc(S * (0.28 + 0.2 * FF.S)));
      if FF.Deep then
        Brush.Color := Col(127, 166, 181)
      else
        Brush.Color := Col(188, 216, 210);
      CA2 := Cos(FF.Angle);
      SA2 := Sin(FF.Angle);
      ...
      LineTo(PX - Trunc(CA2 * RR * 1.4), PY - Trunc(SA2 * RR * 1.4) +
        Trunc(RR * 0.4));
      LineTo(PX - Trunc(CA2 * RR * 1.4), PY - Trunc(SA2 * RR * 1.4) -
        Trunc(RR * 0.4));
    ...
    // ===== FAUNE (vache 0 · loup 1 · sapien 2 · chien 3 · ours 4 · mouton 5) =====
    for C in Creatures do
    begin
      if not C.Alive then
        Continue;
      if not InV(C.X, C.Y, 3) then
        Continue;
      PX := Trunc(OX + C.X * S);
      PY := Trunc(OY + C.Y * S);
      ...
      begin
        ...
        begin
          ...
          if (C.WordT > 0) and (C.Word >= 0) and (C.Word <= 3) then
          begin
        ...
        // — CARTE MENTALE —
        if (FSelected = C) and (Length(C.Mem) > 0) then
        begin
          for MO := 0 to High(C.Mem) do
          begin
            A := 1 - C.Mem[MO].T / 70;
            if A <= 0 then
              Continue;
            ...
            Pen.Style := psSolid;
            Pen.Width := 1;
            if C.Mem[MO].K = 0 then
            begin
              Pen.Color := AlphaColorBlend(Col(201, 106, 69), Col(11, 14, 11),
                Trunc(A * 255));
              RR := Max(3, Trunc(S * 0.7));
              MoveTo(MXp - RR, MYp - RR);
              LineTo(MXp + RR, MYp + RR);
              MoveTo(MXp + RR, MYp - RR);
              LineTo(MXp - RR, MYp + RR);
            end
            else
            begin
              Pen.Color := AlphaColorBlend(Col(157, 187, 107), Col(11, 14, 11),
                Trunc(A * 255));
              RR := Max(3, Trunc(S * 0.7));
              Brush.Style := bsClear;
        ...
        // — LIEN MENTOR —
        if (C.Age < CHILDHOOD) and (C.Mentor <> nil) and (C.Mentor.Alive) then
        begin
          Pen.Style := psDash;
          Pen.Width := 1;
          if FSelected = C then
            ...
            Pen.Color := AlphaColorBlend(Col(208, 167, 92),
              Col(11, 14, 11), 60);
          MoveTo(PX, PY);
        ...
        // — corps détaillé —
        ProRender(FWorld.Canvas, 2, PX, PY, C.Angle, R, Col(211, 172, 138),
          C.HueCol, Phase, Flags);
        // — plume —
        HR := R * (0.5 + 1.7 * C.Orn);
        if HR > 2 then
        begin
          ...
          begin
            ...
            LineTo(PX + K * Trunc(R * 0.85), PY - Trunc(R * 0.5) - Trunc(HR));
          end;
        end;
        // — bulle vocale —
        if (C.WordT > 0) and (C.Word >= 0) and (C.Word <= 3) then
      ...
      else if (C.Kind = 0) or (C.Kind = 5) then
      begin
        // ★Faune — vache (beige) et mouton (toison claire), même gabarit
        if C.Kind = 5 then
          ProRender(FWorld.Canvas, 0, PX, PY, C.Angle, R, Col(242, 238, 228),
            Col(214, 208, 196), Phase, Flags)
        else
          ProRender(FWorld.Canvas, 0, PX, PY, C.Angle, R, Col(228, 222, 206),
            Col(150, 140, 120), Phase, Flags);
        if C.Dom then
        begin
          Pen.Style := psSolid;
          Pen.Width := 1;
          ...
          RR := Trunc(R * 0.8) + 2;
          Ellipse(PX - RR, PY - RR, PX + RR, PY + RR);
          if (C.Kind = 0) and (C.MilkCd <= 0) and (C.Age > 8) then
          begin
            Brush.Style := bsSolid; // ★seule la vache est traite
            Brush.Color := Col(245, 234, 208);
            Ellipse(PX - 2, PY - RR - 6, PX + 2, PY - RR - 3);
          end;
        end
        else if C.Trust > 0 then
        begin
          Pen.Style := psSolid;
          Pen.Width := 1;
      ...
      else if C.Kind = 4 then
      begin
        // ★Faune — l'ours : brun, massif, jamais d'anneau de domestication
        ProRender(FWorld.Canvas, 1, PX, PY, C.Angle, R, Col(97, 69, 47),
          Col(58, 41, 28), Phase, Flags);
      end
      else if C.Kind = 1 then
      begin
        ProRender(FWorld.Canvas, 1, PX, PY, C.Angle, R, Col(194, 106, 69),
          Col(230, 220, 200), Phase, Flags);
        if C.Trust > 0 then
        begin
          Pen.Style := psSolid;
          Pen.Width := 1;
      ...
      begin
        ProRender(FWorld.Canvas, 3, PX, PY, C.Angle, R, Col(206, 168, 104),
          Col(90, 60, 40), Phase, Flags);
        if (C.Master <> nil) and C.Master.Alive then
        begin
          Pen.Style := psDot;
    ...
    // (Phase C, plus tard : la ligne DrawCouronnes ira ICI — juste après ProFlush)

    // ===== SÉLECTION =====
    if (FSelected <> nil) and FSelected.Alive then
    begin
      PX := Trunc(OX + FSelected.X * S);
      PY := Trunc(OY + FSelected.Y * S);
      R := S * (0.55 + 0.55 * FSelected.Sz) *
        (0.55 + 0.45 * Min(1, FSelected.Age / 8));
      Brush.Style := bsClear;
      Pen.Style := psDot;
      Pen.Width := 1;
      Pen.Color := Col(240, 232, 205);
      RR := Trunc(R + 5);
      if RR < 3 then
        RR := 3;
      Ellipse(PX - RR, PY - RR, PX + RR, PY + RR);
    ...
    if FDayLight < 0.98 then
      AlphaFill(FWorld.Canvas, W, H, FTintN, Col(7, 12, 16),
        Trunc((1 - FDayLight) * 105));
    SinD := Sin(FDayT * TAU);
    Warm := Max(0, 1 - Abs(SinD) * 2.5);
    if Warm > 0 then
      AlphaFill(FWorld.Canvas, W, H, FTintW, Col(255, 150, 70),
        Trunc(Warm * 15));
    Froid := SaisonFroid;
    if Froid > 0.03 then
      AlphaFill(FWorld.Canvas, W, H, FTintN, Col(148, 172, 205),
        Trunc(Froid * 38));
    if Froid < 0.97 then
      AlphaFill(FWorld.Canvas, W, H, FTintW, Col(255, 205, 90),
        Trunc((1 - Froid) * 14));
  end;
end;
...
var
  ...
  begin
    Result := Y + 6 + Round(Idx * (BRH - 12) / (NIN - 1))
  end;
  function YH(Idx: Integer): Integer;
  begin
    Result := Y + BRH div 2 + Round((Idx - (NHID - 1) / 2) *
      ((BRH - 20) / (NHID - 1)))
  end;
  function YO(Idx: Integer): Integer;
  begin
    Result := Y + BRH div 2 + Round((Idx - (NOUT - 1) / 2) *
      ((BRH - 70) / (NOUT - 1)))
  end;
...
begin
  ...
  for I := 0 to NIN - 1 do
    ...
    begin
      ...
      if Wv > 0 then
        ...
        C.Pen.Color := AlphaColorBlend(Col(201, 106, 69), Col(17, 21, 15),
          Trunc(A * 255));
      C.MoveTo(XIn, YI(I));
      C.LineTo(XH1, YH(J));
    end;
  ...
  for I := 0 to NIN - 1 do
    ...
    begin
      ...
      if A < 0.05 then
        Continue;
      C.Pen.Style := psDash;
      C.Pen.Color := AlphaColorBlend(Col(120, 130, 100), Col(17, 21, 15),
        Trunc(A * 255));
      C.MoveTo(XIn, YI(I));
      C.LineTo(XOut, YO(O));
      C.Pen.Style := psSolid;
  ...
  begin
    ...
    if FSelected.Inp[I] >= 0 then
      C.Brush.Color := AlphaColorBlend(Col(208, 167, 92), Col(30, 34, 27),
        Trunc((0.15 + 0.85 * A) * 255))
    else
...
begin
  ...
  if (FSelected = nil) or (FSelected.Kind <> 2) or (Length(FSelected.Net) < NW)
  then
  begin
    C.Font.Name := 'Segoe UI';
    C.Font.Size := 11;
...
procedure DrawObservatoire(C: TCanvas; W, H: Integer);
var
  Y, I, J, Car, GN, XR, YInv, YChr: Integer;
  S, Ln: string;
  TT: TTech;
  Items: TStringList;
begin
  ...
  S := Format('JOUR %.2d   %.2d:%.2d   %s · an %d     —     %s',
    ...
    DayCount div JOURS_ANNEE + 1, L(77)]);
  // ★i18n 'clic : sapiens suivant · F7 : revenir'
  C.TextOut(30, 58, S);
  ...
  begin
    ...
    C.Font.Color := Col(139, 138, 116);
      ...
      0: S := L(21); 1: S := L(22);
      5: S := L(162);      // ★faune — mouton
      4: S := L(163);      // ★faune — ours
    else S := L(23);
    end;
    C.TextOut(30, Y, Format('gén. %d · %s · %s', [FSelected.Gen, S,
      FSelected.State]));
    Inc(Y, 26);
    C.Brush.Style := bsClear;
    DrawBar(C, 30, Y, 320, FSelected.Energy / FSelected.MaxE,
      Col(228, 220, 190));
    C.Brush.Style := bsClear;
    C.TextOut(362, Y - 5, L(78));
    Inc(Y, 24); // ★i18n 'énergie'
    if FSelected.Kind = 2 then
    begin
      DrawBar(C, 30, Y, 320, FSelected.Cult, Col(157, 187, 107));
      C.Brush.Style := bsClear;
      C.TextOut(362, Y - 5, L(67));
      Inc(Y, 24); // ★i18n 'savoir'
    end;
    DrawBar(C, 30, Y, 320, (FSelected.Sp - 2.2) / 3.2, Col(208, 167, 92));
    C.Brush.Style := bsClear;
    C.TextOut(362, Y - 5, L(64));
    Inc(Y, 24); // ★i18n 'vitesse'
    DrawBar(C, 30, Y, 320, (FSelected.Se - 4) / 8, Col(208, 167, 92));
    C.Brush.Style := bsClear;
    C.TextOut(362, Y - 5, L(65));
    Inc(Y, 24); // ★i18n 'perception'
    DrawBar(C, 30, Y, 320, (FSelected.Sz - 0.6) / 0.9, Col(208, 167, 92));
    C.Brush.Style := bsClear;
    C.TextOut(362, Y - 5, L(66));
    Inc(Y, 28); // ★i18n 'taille'
    if FSelected.Kind = 2 then
    begin
      ...
      try
        ...
        begin
          ...
          begin
            if Ln = '' then
              Ln := Items[I]
            else if C.TextWidth(Ln + ' · ' + Items[I]) <= 400 then
              Ln := Ln + ' · ' + Items[I]
            else
            begin
              C.TextOut(30, Y, Ln);
  ...
  begin
    ...
    for YChr := 0 to Creatures.Count - 1 do
      if Creatures[YChr].Alive and (Creatures[YChr].Kind = 2) and
        (TT in Creatures[YChr].Tech) then
        Inc(Car);
    if GN > 0 then
      Car := Round(Car / GN * 100)
    else
      Car := 0;
    YInv := Y + (J div 2) * 16;
    C.Font.Color := Col(240, 180, 95);
  ...
  if Length(InnoLog) = 0 then
    ...
    for I := Max(0, High(InnoLog) - 7) to High(InnoLog) do
    begin
      if (InnoLog[I].Word >= 0) and (InnoLog[I].Word <= 3) then
        C.Font.Color := WORDCOL[InnoLog[I].Word]
      else
        C.Font.Color := Col(208, 167, 92);
      C.TextOut(XR, YInv, Format('%s — %s (j.%d)', [InnoLog[I].Base,
        InnoLog[I].Who, InnoLog[I].Day]));
      Inc(YInv, 16);
    end;
  ...
  if Length(Chronicle) > 0 then
    for I := Max(0, Length(Chronicle) - 10) to Length(Chronicle) - 1 do
    begin
      if YChr > H - 30 then
        Break;
      case Chronicle[I].Kind of
        ...
        CK_INNO:
          C.Font.Color := Col(208, 167, 92);
        CK_LIFE:
          C.Font.Color := Col(170, 160, 140);
      else
        C.Font.Color := Col(157, 187, 107);
      end;
      C.TextOut(XR, YChr, Format('j.%d · %s', [Chronicle[I].Day,
        Chronicle[I].Text]));
      Inc(YChr, 15);
    end;
...
var
  ...
  begin
    ...
    if not Mesuring then
      AddBtn(Rect(BX, Y - FPanelScroll, BX + Wd, Y - FPanelScroll + 26), Cap,
        Id, Active);
    C.Font.Name := 'Segoe UI';
    C.Font.Size := 9;
    C.Font.Style := [];
...
begin
  ...
  Y := 18;
  C.Font.Name := 'Segoe UI';
  C.Font.Size := IfThen(FPanelW > 320, 13, 10);
  C.Font.Style := [];
  C.Font.Color := Col(230, 224, 205);
  IdxS := (DayCount mod JOURS_ANNEE) * 4 div JOURS_ANNEE;
  S := Format('JOUR %.2d   %.2d:%.2d   %s · an %d',
    [DayCount, Trunc(FDayT * 24), Trunc(Frac(FDayT * 24) * 60), SAISNOM[IdxS],
    DayCount div JOURS_ANNEE + 1]);
  C.TextOut(20, Y, S);
  Inc(Y, IfThen(FPanelW > 320, 26, 20));

  C.Font.Name := 'Georgia';
  C.Font.Size := IfThen(FPanelW > 320, 15, 11);
  C.Font.Style := [fsItalic, fsBold];
  C.Font.Color := Col(208, 167, 92);
  ...
  C.Font.Style := [];
  Inc(Y, IfThen(FPanelW > 320, 26, 20));
  if (FEra < ERE_MAX) and (not CanPassEre) then
  begin
    C.Font.Color := Col(100, 105, 88);
    S := Format(L(32), [CountTechEre(FEra), IfThen(FEra = 1, 7, 8), CountS,
      CountInno, InnoTotal]);
    C.TextOut(20, Y, S);
    ...
    case I of
      ...
      5:
        begin
          S := L(162);
          V := CountM;
          C.Brush.Color := Col(242, 238, 228)
        end; // ★faune
      6:
        begin
          S := L(163);
          V := CountO;
          C.Brush.Color := Col(97, 69, 47)
        end; // ★faune
    else
      begin
        S := L(24);
        V := FsN + FdN;
        C.Brush.Color := Col(159, 196, 207)
      end;
    end;
    C.Brush.Style := bsSolid;
  ...
  for I := 0 to Creatures.Count - 1 do
    if Creatures[I].Alive and (Creatures[I].Kind = 2) then
    begin
      CS := CS + Creatures[I].Cult;
      Inc(GN);
    end;
  C.Font.Color := Col(157, 187, 107);
  if GN > 0 then
    S := Format(L(27), [Round(CS / GN * 100)])
  else
    S := L(28);
  C.TextOut(20, Y, S);
  ...
  for I := 0 to Creatures.Count - 1 do
    if Creatures[I].Alive and (Creatures[I].Kind = 2) then
    begin
      if tNav in Creatures[I].Tech then
        Inc(Nv);
      if IsWater(Creatures[I].X, Creatures[I].Y) then
        Inc(AtSea);
    end;
  ...
  begin
    ...
    if TECHBASE[I].Era < EreCourante then
    begin
      if TECHBASE[I].Era <> LastEra then
      begin
    ...
    for J := 0 to Creatures.Count - 1 do
      if Creatures[J].Alive and (Creatures[J].Kind = 2) and
        (TT in Creatures[J].Tech) then
        Inc(Car);
    if GN > 0 then
      Car := Round(Car / GN * 100)
    else
      Car := 0;
    if TechInfo[TT].Who <> '' then
    begin
      if Car > 0 then
  ...
  C.Pen.Style := psClear;
  C.FillRect(R);
  if Length(FHist) > 1 then
  begin
    MaxV := 10;
    for I := 0 to High(FHist) do
    begin
      if FHist[I].P > MaxV then
        MaxV := FHist[I].P;
      if FHist[I].H > MaxV then
        MaxV := FHist[I].H;
      if FHist[I].C > MaxV then
        MaxV := FHist[I].C;
      if FHist[I].S > MaxV then
        MaxV := FHist[I].S;
    end;
    for I := 0 to 3 do
      DrawLine(C, R, MaxV, I);
  ...
  begin
    Has := Lex[I].N > 0.8;
    if Has then
      PP := Round(Lex[I].Pred / Lex[I].N * 100)
    else
      PP := 0;
    if Has then
      FF := Round(Lex[I].Food / Lex[I].N * 100)
    else
      FF := 0;
    Gl := '—';
    if Has then
    begin
      if (PP > 55) and (PP >= FF) then
        Gl := L(60)
      else if FF > 55 then
        Gl := L(61)
      else if (PP < 22) and (FF < 22) then
        Gl := L(62);
    end;
  ...
  begin
    for I := Max(0, High(InnoLog) - 11) to High(InnoLog) do
    begin
      C.Brush.Style := bsClear;
      if (InnoLog[I].Word >= 0) and (InnoLog[I].Word <= 3) then
        C.Font.Color := WORDCOL[InnoLog[I].Word]
  ...
  begin
    Section(L(12));
    C.Font.Name := 'Georgia';
    C.Font.Size := IfThen(FPanelW > 320, 18, 14);
    C.Font.Style := [fsItalic, fsBold];
    C.Font.Color := FSelected.HueCol;
    C.TextOut(20, Y, FSelected.Name);
    if not Mesuring then
    begin
      AddBtn(Rect(PW - 84, Y - FPanelScroll + 2, PW - 20,
        Y - FPanelScroll + 24), L(19), BID_Info, False);
      C.Brush.Style := bsSolid;
      C.Pen.Style := psSolid;
      C.Pen.Width := 1;
      C.Pen.Color := Col(38, 43, 33);
    ...
    Inc(Y, 12);
    C.TextOut(20, Y, L(64));
    DrawBar(C, 94, Y + 2, PW - 148, (FSelected.Sp - 2.2) / 3.2,
      Col(208, 167, 92));
    Inc(Y, 12);
    C.TextOut(20, Y, L(65));
    ...
    Inc(Y, 12);
    C.TextOut(20, Y, L(66));
    DrawBar(C, 94, Y + 2, PW - 148, (FSelected.Sz - 0.6) / 0.9,
      Col(208, 167, 92));
    Inc(Y, 12);
    if FSelected.Kind = 2 then
    begin
      ...
      Inc(Y, 12);
      S := '';
      if tFeu in FSelected.Tech then
        S := S + TechNom(tFeu) + ' ';
      if tAgri in FSelected.Tech then
        S := S + TechNom(tAgri) + ' ';
      if tStock in FSelected.Tech then
        S := S + TechNom(tStock) + ' ';
      if tPast in FSelected.Tech then
        S := S + TechNom(tPast) + ' ';
      if tPeche in FSelected.Tech then
        S := S + TechNom(tPeche) + ' ';
      if tNav in FSelected.Tech then
        S := S + TechNom(tNav) + ' ';
      if S <> '' then
      begin
  ...
  BX := 20;
  if FRunning then
    Btn('II', BID_PLAY, (PW - 58) div 4, True)
  else
    Btn('>', BID_PLAY, (PW - 58) div 4, False);
  ...
  begin
    ...
    begin
      for J := Max(0, Length(Chronicle) - 14) to Length(Chronicle) - 1 do
      begin
        C.Brush.Style := bsClear;
        ...
        C.TextOut(20, Y, Format('j.%d · %s', [Chronicle[J].Day,
          Chronicle[J].Text]));
        Inc(Y, IfThen(FPanelW > 320, 18, 14));
      end;
      Inc(Y, 8);
    ...
    begin
      ...
      case Creatures[I].Kind of
        ...
        1:
          C.Brush.Color := Col(201, 106, 69);
        3:
          C.Brush.Color := Col(206, 168, 104);
      else
        C.Brush.Color := Col(208, 167, 92);
      end;
      C.FillRect(Rect(20 + Trunc(Creatures[I].X / GW * IfThen(FPanelW > 320,
        160, 100)) - 1, Y + Trunc(Creatures[I].Y / GH * IfThen(FPanelW > 320,
        99, 62)) - 1, 20 + Trunc(Creatures[I].X / GW * IfThen(FPanelW > 320,
        160, 100)) + 1, Y + Trunc(Creatures[I].Y / GH * IfThen(FPanelW > 320,
        99, 62)) + 1));
    end;
  end;
